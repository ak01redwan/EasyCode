  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light mb-3">
    <div class="container-fluid" v-if="user">
      ...
      <div class="dropdown me-3">
        <button class="btn btn-secondary dropdown-toggle" type="button" id="filter-dropdown" data-bs-toggle="dropdown"
          aria-expanded="false">
          Filter: {{ filter }}
        </button>
      ...
      <button v-if="user.userType == 'admin'" class="btn btn-primary" data-bs-toggle="modal"
      ...
      <div class="col-12" style="
        ...
        <div v-for="(course, index) in searchResults" :key="index" class="user-card">
          ...
          <div class="btn-group float-end">
            <button v-if="user.userType == 'admin'" @click="addCourseAdmin(course)" data-bs-toggle="modal"
              data-bs-target="#addCourseAdmin" class="btn btn-outline-secondary" 
              :title="course.courseAdmin ? 'change course admin' : 'Add Course Admin'">
              <i v-if="!course.courseAdmin" class="fa-solid fa-plus text-bold text-success"></i>
              <i v-else class="fa-solid fa-circle text-warning"></i>
            </button>
            <button @click="toggleCoursePublished(course.id)" v-if="!course.isPublished" class="btn btn-outline-secondary"
              title="Publish this course">
              <i class="fa-solid fa-upload"></i>
            </button>
            <button @click="toggleCoursePublished(course.id)" v-else-if="course.isPublished"
              class="btn btn-outline-secondary" title="Unpublish this course">
              <i class="fa-sharp fa-regular fa-circle-stop"></i>
            </button>
            <button @click="viewCourseDetails(course)" class="btn btn-outline-info" title="View Details">
              <i class="fas fa-info-circle"></i>
            </button>
            <button v-if="user.userType == 'admin'" @click="deleteThisCourse(course)" class="btn btn-outline-danger"
